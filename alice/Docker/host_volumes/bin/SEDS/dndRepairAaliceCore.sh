...
sed -i "s/\.*self.ThreadManager.doLater(interval=2,.*func=self\.systemUpdate)//" ~/ProjectAlice/skills/AliceCore/AliceCore.py


# In def stopListenIntent
sed -i "s/\.*self.Commons.runRootSystemCommand(\['systemctl', 'stop', 'snips-hotword'\])/self.notifyDevice(constants.TOPIC_DND, siteId=session.siteId)\n\t\t\tself.Commons.runRootSystemCommand(\['systemctl', 'stop', 'snips-hotword'\])/" ~/ProjectAlice/skills/AliceCore/AliceCore.py



# In def unmuteSite
sed -i "s/\.*self.Commons.runRootSystemCommand(\['systemctl', 'start', 'snips-hotword'\])/self.notifyDevice(constants.TOPIC_STOP_DND, siteId=siteId)\n\t\t\tself.Commons.runRootSystemCommand(\['systemctl', 'start', 'snips-hotword'\])/" ~/ProjectAlice/skills/AliceCore/AliceCore.py
